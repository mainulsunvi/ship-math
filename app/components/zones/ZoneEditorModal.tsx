 ...
 * Country multi-select (common countries + worldwide "*"), province list
 * when exactly one country is selected, and the postal rules list built from
 * PostalRuleEditor. Submits `zone-create` / `zone-update` to the owning
 * route with useFetcher; the action re-validates the JSON columns and pushes
 * the function mirror.
 */
...
export default function ZoneEditorModal({ zone, onClose }: ZoneEditorModalProps) {
    ...
    fetcher.submit(
      {
        ...
        postalRules: JSON.stringify(postalRules),
      },
      { method: "post" },
    );
  }
  ...
  const nameError =
    submitResult && submitResult.fieldErrors ? submitResult.fieldErrors.name : undefined;
  const postalError =
    submitResult && submitResult.fieldErrors ? submitResult.fieldErrors.postal : undefined;
